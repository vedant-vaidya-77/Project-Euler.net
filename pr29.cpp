...
class BigInt {
private:
    string number; // To store the number as a string
    bool isNegative; // To store the sign of the number
...
public:
    ...
    BigInt operator*(const BigInt& other) const {
        ...
        for (int i = len1 - 1; i >= 0; --i) {
            ...
            for (int j = len2 - 1; j >= 0; --j) {
                int temp = (result[i + j + 1] - '0') + (number[i] - '0') * (other.number[j] - '0') + carry;
    ...
    bool operator<(const BigInt& other) const {
        ...
        if (number.size() != other.number.size()) {
            return isNegative ? number.size() > other.number.size() : number.size() < other.number.size();
        }

        return isNegative ? number > other.number : number < other.number;
    }
...
int solve() {
    set<BigInt> uniquePowers;

    for (int a = 2; a <= 100; ++a) {
        BigInt base = BigInt(a);
        BigInt result = base;

        for (int b = 2; b <= 100; ++b) {
            result = result * base;
            uniquePowers.insert(result);
        }
    }

    cout << uniquePowers.size() << endl;
