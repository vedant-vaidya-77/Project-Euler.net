...
int solve(){
    // can be solved by calling nextpermutation 1 miliion times, barely making in time
    string real = "0123456789";
    int factorials[13+1] = {1, 1, 2, 6, 24, 120, 720, 5040, 40320, 362880, 39916800, 479001600, 6227020800};
    int n;
    cin>>n;
    n %= factorials[real.size()];
    n--;
    string rem = real;
    string res = "";
    while(!rem.empty()){
    	int curr = factorials[rem.size() - 1];
    	int pos = n/curr;
    	res += rem[pos];
    	rem.erase(pos, 1);
    	n %= curr;
